...
def initialize_components(config):
    ...
    bt.logging.info(
        ...
        f"on network: {config.subtensor.chain_endpoint}"
    )
    ...
    for retry in range(max_retries):
        try:
            ...
            answer = await call_openai(
                messages, 0.33, "gpt-3.5-turbo", random_seed
            )
            ...
            if extracted_list:
                ...
                bt.logging.info(
                    f"No valid python list found, retry count: {retry + 1}"
                )
        except Exception as e:
            retry += 1
            bt.logging.error(
                f"Got exception when calling openai {e}\n"
                f"{traceback.format_exc()}"
            )

    bt.logging.error(
        f"No list found after {max_retries} retries, using default list."
    )
    return default
...
async def update_counters_and_get_new_list(category, item_type, theme=None):
    ...
    async with list_update_lock:
        ...
        bt.logging.debug(
            f"Queue for {list_type}: {len(items) if items else 0} items"
        )
        ...
        if not items:
            ...
            bt.logging.debug(
                f"Fetched new list for {list_type}, "
                f"containing {len(items)} items"
            )
...
async def check_uid(dendrite, axon, uid):
    ...
    try:
        response = await dendrite(
            axon, IsAlive(), deserialize=False, timeout=2.3
        )
...
async def get_available_uids(dendrite, metagraph):
    ...
    tasks = [
        check_uid(dendrite, metagraph.axons[uid.item()], uid.item())
...
def set_weights(scores, config, subtensor, wallet, metagraph):
    ...
    moving_average_scores = (
        alpha * scores + (1 - alpha) * moving_average_scores
    )
    bt.logging.info(
        f"Updated moving average of weights: {moving_average_scores}"
    )
    subtensor.set_weights(
        netuid=config.netuid,
        wallet=wallet,
        uids=metagraph.uids,
        weights=moving_average_scores,
...
async def get_and_score_images(
    dendrite,
    metagraph,
    config,
    subtensor,
    ...
    for uid in available_uids:
        ...
        # if wanting a specific image, redefine messages here with
        # an image prompt and comment out the above line
        # messages = (
        #     "aquamarine and pink, religious symbolism, american works "
        #     "on paper 1880–1950, flowerpunk, colorful assemblages"
        # )
        ...
        task = query_image(
            dendrite, metagraph.axons[uid], uid, syn, config, subtensor, wallet
        )
    ...
    for uid, response in query_responses:
        if response:
            ...
            if completion is not None:
                ...
                task = template.reward.image_score(
                    uid, image_url, size, messages_for_uid, weight
                )
    ...
    bt.logging.info(
        f"Scoring tasks completed for UIDs: {[uid for uid, _ in score_tasks]}"
    )
...
async def query_text(dendrite, axon, uid, syn, config, subtensor, wallet):
    try:
        bt.logging.info(
            f"Sent query to uid: {uid}, {syn.messages[0]['content']} "
            f"using {syn.engine}"
        )
        full_response = ""
        responses = await asyncio.wait_for(
            dendrite([axon], syn, deserialize=False, streaming=True), 20
        )
        ...
        bt.logging.error(
            f"Exception during query for uid {uid}: {e}\n"
            f"{traceback.format_exc()}"
        )
        return uid, None


async def get_and_score_text(
    dendrite,
    metagraph,
    config,
    subtensor,
    ...
    bt.logging.info(
        f"Random Number: {random_number}, Will Score All: {will_score_all}"
    )
    ...
    openai_responses = await asyncio.gather(
        *[task for _, task in openai_tasks]
    )
    ...
    for (uid, _), openai_answer in zip(openai_tasks, openai_responses):
        if openai_answer:
            ...
            task = template.reward.openai_score(
                openai_answer, response, weight
            )
            score_tasks.append((uid, task))
...
async def query_synapse(dendrite, metagraph, subtensor, config, wallet):
    ...
    while True:
        try:
            ...
            if steps_passed % 2 != 1:
                scores, uid_scores_dict = await get_and_score_text(
                    dendrite,
                    metagraph,
                    config,
                    subtensor,
                ...
                scores, uid_scores_dict = await get_and_score_images(
                    dendrite,
                    metagraph,
                    config,
                    subtensor,
            ...
            total_scores += scores
            bt.logging.info(
                f"scores = {uid_scores_dict}, {2 - steps_passed % 3} "
                f"iterations until set weights"
            )
